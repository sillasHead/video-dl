...
try {
    ...
    # Remove a instalação legada criada antes do instalador atual.
    if (Test-Path -LiteralPath $legacyDir) {
        Remove-Item -LiteralPath $legacyDir -Recurse -Force -ErrorAction SilentlyContinue
    }

    # Remove wrappers antigos do perfil do Windows PowerShell/PowerShell 7.
    $profiles = @(
    ...
    # Substitui a aplicação, mas preserva %USERPROFILE%\.video-dl (config/dependências).
    ...
    # Garante que o comando video-dl esteja disponível para o usuário.
    ...
    # Evita que uma função antiga carregada neste terminal ganhe prioridade sobre o .cmd novo.
